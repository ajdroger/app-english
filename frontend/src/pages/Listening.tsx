...
// English-only starter phrases — for other languages, AI generates them
...
export default function Listening() {
  ...
  // Reset and load phrases when language changes
  useEffect(() => {
    setFeedback(null)
    setStep('idle')
    setPhraseIndex(0)
    ...
    if (language.code === 'english') {
      setPhrases(ENGLISH_PHRASES)
    } else {
      setPhrases([])
      setLoadingPhrases(true)
      axios.post('/api/listening/generate', {
  ...
  const startRecording = async () => {
    ...
    mr.onstop = async () => {
      ...
      form.append('target_language', language.code)
      form.append('native_language', nativeLanguage.code)
      setStep('loading')
      try {
    ...
    <div className="flex flex-col items-center gap-6 max-w-2xl mx-auto">
      <div className="w-full flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">
          {language.flag} Pronunciation Practice
        </h1>
        <button
      ...
      {showGen && (
        <div className="w-full bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-5 flex flex-col gap-4 border border-indigo-100 dark:border-indigo-900">
          <h3 className="font-semibold text-gray-800 dark:text-gray-100 text-sm">
            Generate {language.label} phrases with AI
          </h3>
          ...
          {generating && <p className="text-indigo-400 text-sm animate-pulse">Generating {language.label} phrases with AI…</p>}
          {genMsg && <p className="text-sm font-medium">{genMsg}</p>}
        </div>
      )}

      {/* Loading starter phrases */}
      {loadingPhrases && (
        <div className="w-full bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-10 flex flex-col items-center gap-3">
          <p className="text-indigo-400 animate-pulse text-sm">
            Loading {language.label} phrases…
          </p>
        </div>
      )}
      ...
      {!loadingPhrases && phrases.length === 0 && (
        <div className="flex flex-col items-center gap-4 mt-4 text-center max-w-sm">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            Could not load phrases automatically. Use ✨ Generate phrases to add some.
          </p>
        </div>
      )}
